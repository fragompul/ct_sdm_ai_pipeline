...
def run_baseline_inference(target_specs, config, device):
    ...
    # 2. Inferencia en RNN Específica [cite: 1094, 1284]
    start_rnn = time.time()
    ...
    # Redefinir la clase temporalmente para cargarla
...
def run_new_pipeline_inference(target_specs, config, device):
    ...
    # Usaremos TabularDDPM que fue tu ganador en la Fase 3
    gen_model = TabularDDPM(spec_dim=len(spec_cols), super_vector_dim=super_vector_dim, hidden_dim=256).to(device)
    gen_model.load_state_dict(torch.load(os.path.join(models_dir, 'phase3_tabularddpm.pth'), map_location=device))
    gen_model.eval()
    
    # Asumiendo ResNet o DeepMLP como ganador de Fase 4 (Ajusta el nombre si fue XGBoost)
    surrogate_model = SurrogateResNet(super_vector_dim=super_vector_dim, output_metrics=len(spec_cols), hidden_dim=256).to(device)
    surrogate_model.load_state_dict(torch.load(os.path.join(models_dir, 'phase4_resnet.pth'), map_location=device))
    ...
    cost_matrix = np.array([0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 0.1, 0.6, 1.1, 1.6, 2.1, 2.6]) # Extraída dinámicamente en tu código
    ...
    for top_idx in top3_indices:
        ...
        # Generar semilla desde Difusión (simplificado con t=0 para inferencia rápida)
        with torch.no_grad():
        ...
        print(f"   ⚙️  ID {topology_id} optimizado en {p34_time:.2f} ms -> FoMs Predictivo Máximo: {best_fom:.2f} dB")
